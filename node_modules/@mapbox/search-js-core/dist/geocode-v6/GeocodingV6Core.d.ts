...
interface PermanentOptions {
     ...
     * Separate billing for permanent geocoding may apply.
     *
...
/**
 * A `GeocodingV6Core` object is an application's main entrypoint to the [Mapbox Geocoding API](https://docs.mapbox.com/api/search/geocoding/).
 ...
 * methods from the `GeocodingV6Core` object.
 *
 * A [Mapbox access token](https://docs.mapbox.com/help/glossary/access-token/) is required to use `GeocodingV6Core`, and
 * other options may be specified either in the constructor or in the {@link GeocodingV6Core#forward} or {@link GeocodingV6Core#reverse} calls.
 *
 * @class GeocodingV6Core
 *
 * @example
 * ```typescript
 * const geocode = new GeocodingV6Core({ accessToken: 'pk.my-mapbox-access-token' });
 *
 ...
 * ```
 */
export declare class GeocodingV6Core {
    #private;
    /**
     * The [Mapbox access token](https://docs.mapbox.com/help/glossary/access-token/) to use for all requests.
     */
     ...
     * - {@link GeocodingV6Core#forward}
     * - {@link GeocodingV6Core#reverse}
     */
    defaults: Partial<GeocodingOptions>;
    constructor(options?: Partial<AccessTokenOptions & GeocodingOptions & PermanentOptions>);
    /** @section {Methods} */
    /**
     * {@link GeocodingV6Core#forward} allows you to look up a single location by name
     ...
     * calls to {@link GeocodingV6Core#forward} should specify `permanent: true` within the method options argument.
     *
    ...
    optionsArg?: Partial<FetchOptions & GeocodingOptions & PermanentOptions>): Promise<GeocodingResponse>;
    /**
     * {@link GeocodingV6Core#reverse} allows you to look up a single pair of coordinates and returns the
     ...
     * Due to legal terms from our data sources, geographic coordinates should be used ephemerally and not persisted.
     * If the results are to be cached/stored in a customer database,
     * calls to {@link GeocodingV6Core#reverse} should specify `permanent: true` within the method options argument.
     *
     * This permanent policy is consistent with the [Mapbox Terms of Service](https://www.mapbox.com/tos/) and failure to comply
     * may result in modified or discontinued service.
     *
    ...
    suggest(searchText: string, optionsArg?: Partial<FetchOptions & GeocodingOptions & PermanentOptions>): Promise<GeocodingResponse>;
